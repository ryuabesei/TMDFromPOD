...
# 軸設定
ax1.set_title(f"Swarm-B normalized density ({SECTOR_LEFT[0]:02d}–{SECTOR_LEFT[1]:02d} LT)")
ax2.set_title(f"Swarm-B normalized density ({SECTOR_RIGHT[0]:02d}–{SECTOR_RIGHT[1]:02d} LT)")
...
print("N left :", len(df_left))
print("N right:", len(df_right))
# --- Swarm-B のLT分布を確認（どのLT帯にデータがあるか）---
print("lst_h min/max:", df["lst_h"].min(), df["lst_h"].max())

bins = np.arange(0, 25, 1)
hist, edges = np.histogram(df["lst_h"].to_numpy(), bins=bins)

print("LT histogram (1h bins):")
for i in range(24):
    if hist[i] > 0:
        print(f"{i:02d}-{i+1:02d} LT :", hist[i])
