"""
plot_2d_residual_SWARM-B_DOY20-80.py

目的:
    1. DOY 20-40 & 61-80 の正規化密度を各緯度ビンで平均 → non-SSW reference profile(lat)
    2. DOY 20-80 の 2D グリッドから reference を差し引いて residual を計算
...
OUT_PNG      = Path("Figure/2018/2D_residual_SWARM-B_DOY20-80_LT22-5_11-17_450km.png")

T_START = "2018-01-20 00:00:00"   # DOY 20
T_END   = "2018-03-21 23:59:59"   # DOY 80

SECTOR_NIGHT = (22, 5)     # 22–05 LT (深夜またぎ)
SECTOR_DAY   = (11, 17)    # 11–17 LT
MORNING_WRAPS = True       # モーニングセクターが深夜をまたぎ
...
def compute_residual_grid(
    ...
    """
    DOY 20-40 & 61-80 の列を平均して reference profile(lat) を作り、
    Z_full から差し引く。
    """
...
# =========================
def main() -> None:
    print("Loading normalized data (SWARM-B, DOY 20-80) ...")
    ...
    print(f"  night (22–05 LT): {len(df_n):,}, day (11–17 LT): {len(df_d):,}")
    ...
    fig.suptitle(
        ...
        "Reference: DOY 20–40 & 61–80 mean per latitude (non-SSW, ref: 450 km)",
    ...
    panels = [
        (0, Z_res_n, df_n, SECTOR_NIGHT, "Residual (22–05 LT)"),
        (1, Z_res_d, df_d, SECTOR_DAY,   "Residual (11–17 LT)"),
    ]
    ...
    for col, Z_res, df_sec, sector, title in panels:
        ...
        # Reference 帯をハイライト
        ax.axvspan(DOY_REF1_MIN, DOY_REF1_MAX, color="lightblue", alpha=0.15, lw=0,
                   label="Non-SSW reference")
        ax.axvspan(DOY_REF2_MIN, DOY_REF2_MAX, color="lightblue", alpha=0.15, lw=0)
        ...
        if sector == SECTOR_NIGHT and MORNING_WRAPS:
            ax_r.set_ylim(22, 29)
            ax_r.set_yticks([22, 23, 24, 25, 26, 27, 28, 29])
            ax_r.set_yticklabels(["22", "23", "0", "1", "2", "3", "4", "5"])
            x_lt, y_lt = daily_representative_lt_line_wrap(df_sec, lt_min, lt_max, stat="median")
        else:
