"""
plot_1D_delta_ratio_only_2019_by_LT.py

Purpose:
    Plot daily-median delta_ratio (Residual = ratio - median(non-SSW ref)) split by LT sector
    for SWARM-A, B, C during the 2019 SH SSW (2019-08-20 to 2019-09-23) [delta only].
...
Output:
    Figure/2019/1D_delta_ratio_only_msis_2019_SWARM-{A,B,C}_by_LT.png
"""
...
SATELLITES = [
    dict(
        ...
        out_png    = Path("Figure/2019/1D_delta_ratio_only_msis_2019_SWARM-A_by_LT.png"),
    ...
    dict(
        ...
        out_png    = Path("Figure/2019/1D_delta_ratio_only_msis_2019_SWARM-B_by_LT.png"),
    ...
    dict(
        ...
        out_png    = Path("Figure/2019/1D_delta_ratio_only_msis_2019_SWARM-C_by_LT.png"),
        lt_sectors = LT_DAWN_DUSK,
    ),
]
...
def plot_satellite(sat: dict) -> None:
    ...
    for col_idx, lt in enumerate(lt_sectors):
        ...
        for bi, (band_label, lat_lo, lat_hi) in enumerate(LAT_BANDS):
            ...
            ax.plot(residual.index, residual.values, color=color, linewidth=2.0, marker="o", markersize=4, zorder=4,
                    label=f"delta_ratio ({band_label.strip()})")
    ...
    fig.suptitle(f"{label}  density_ratio_msis residual (2019 SH SSW)\n{lt_names}",
